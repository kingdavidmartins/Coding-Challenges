// Meet Fibonacci bigger brother, AKA Tribonacci.
//
...
// Signature will always contain 3 numbers; n will always be a non-negative number; if n==0, then return an empty array and be ready for
// anything else which is not clearly specified ;)

const tribonacci = (arr, n) => {
  let tri = [arr[0], arr[1], arr[2]];
  for (let a = 0; a < n; a ++){
    tri.push((tri[a] + tri[a + 1] + tri[a + 2]))
  }
  return tri.filter((num, i) => i < n );
}
