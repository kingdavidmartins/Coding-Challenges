...
// To help them in their goal, the game will accept guesses, and
// return the number of letters that are in the correct position.
...
const countCorrectCharacters = (correctWord, guess) =>
  (correctWord.length !== guess.length)
    ...
    : correctWord
        .split('')
        .map((letter, indexofLetter, letterArr) =>
          (letterArr[indexofLetter] === guess.split('')[indexofLetter])
            ? 1
            : 0)
        .reduce((a, b) => a + b)

console.log(
  countCorrectCharacters('dog', 'god')
);
