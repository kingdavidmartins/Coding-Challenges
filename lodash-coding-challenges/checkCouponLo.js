...
var checkCoupon = (enteredCode, correctCode, currentDate, expirationDate) => {
  if (enteredCode !== correctCode) {
		...
		var currentDateArray = _.chain(currentDate.split(" ")).map((value, index) => (index === 1) ? value.slice(0, 1) : value).map((value, index) => (index > 0) ? parseFloat(value) : value).value();
		var expirationDateArray = _.chain(expirationDate.split(" ")).map((value, index) => (index === 1) ? value.slice(0, 1) : value).map((value, index) => (index > 0) ? parseFloat(value) : value).value();
